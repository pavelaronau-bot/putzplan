...
def _require_row(row, message: str = "запрос не вернул строку"):
    """SQL-функции ниже всегда возвращают ровно одну строку.

    None означает нарушение контракта БД и должно падать явно,
    а не превращаться в None.
    """
    if row is None:
...
class UserRepository:
            ...
            await self.session.execute(
                text(
                    ...
                    VALUES (
                        ...
                        CAST(:hash AS text),
                        CASE
                            WHEN CAST(:hash AS text) IS NULL THEN NULL
                            ELSE now()
                        END,
                        CAST(:st AS user_status),
                        :by
                    )
                    RETURNING
                        id,
        ...
        row = (
            await self.session.execute(
                ...
                ),
                {
                    ...
                    "p": position,
                    "ph": phone,
                    "r": str(role_id) if role_id else None,
                    "by": str(updated_by),
                },
            )
        ).mappings().first()
    ...
    async def revoke_session_of_user(
        self,
        user_id: UUID,
        session_id: UUID,
        reason: str,
